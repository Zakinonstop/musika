    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.video, function (i, data) {
                $('#menu-materi').append(
                    `
                    <div class="accordion-item mb-3 border-bottom" data-order="` +
                        data.order +
                        `" data-video="` +
                        data.video_id +
                        `">
                        <h2 class="accordion-header btn-list-materi order-` +
                        data.order +
                        `" id="flush-materi" data-video="` +
                        data.video_id +
                        `">
                            <button class="accordion-button accordion-button-primary bg-white text-dark collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse` +
                        data.order +
                        `" aria-expanded="false" aria-controls="flush-collapseOne">
                                <div class="row">
                                    <div class="col-3">
                                        <img src="` +
                        data.thumbnail +
                        `" alt="" style="width: 50px;" class="materi-thumbnail">
                                    </div>
                                    <div class="col-9">
                                        <div class="fw-bold">` +
                        data.title +
                        ...
                        data.order +
                        `" class="accordion-collapse collapse" aria-labelledby="flush-materi" data-bs-parent="#menu-materi">
            ...
            $('.btn-list-materi').click(function () {
                ...
                $.ajax({
                    url:
                        'https://lms.lazy2.codes/public/api/course/video/' +
                        video,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function (result) {
                    ...
                    error: function (result, ajaxOptions, thrownError) {
                        ...
                        $(
                            '.video1, .card-next-prev, .loader, .skeleton-video'
                        ).addClass('hide');
